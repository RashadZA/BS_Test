...
class RepoListSortByStarTable {
  String repositorySortByStar = 'repositorySortByStar';
  ...
  Future getRepoListSortByStar() async {
    Database db = await DatabaseHelper().database;
    var result = await db.query(repositorySortByStar, orderBy: '$primeKey ASC');
    // print('Get RepoList result: $result');
    return result;
  }


  Future insertRepoListSortByStar({
    int? primeKeyRepo,
    int? idRepo,
    String? nodeIdRepo,
    String? nameRepo,
    ...
    var result = await db.insert(repositorySortByStar, row,conflictAlgorithm: ConflictAlgorithm.replace);
    // print('Insert RepoList result: $result');
    return result;
  }
  getRepoSBSByPrimaryKey(int key) async {
    Database db = await DatabaseHelper().database;
    var result = await db.rawQuery("SELECT * FROM $repositorySortByStar WHERE $primeKey = $key");
    // print("getRepoByPrimaryKey:- result = $result");
    return result;
  }
  getRepoSBSByPrimaryKeys({required int fromKey, required int toKey}) async {
    Database db = await DatabaseHelper().database;
    var result = await db.rawQuery("SELECT * FROM $repositorySortByStar WHERE $primeKey BETWEEN $fromKey AND $toKey");
    // print("getRepoByPrimaryKeys:- fromKey = $fromKey and toKey = $toKey");
    return result;
  }
  getRepoSBSByURL(String repoUrl) async {
    Database db = await DatabaseHelper().database;
    var result = await db.rawQuery("SELECT * FROM $repositorySortByStar WHERE $url = '${repoUrl.toString()}'");
    // print("getRepoByPrimaryKey:- result = $result");
    return result[0];
  }
  Future updateRepoSBS({int? idRepo}) async {
    Database db = await DatabaseHelper().database;
    var row = {
      id: idRepo
    };
    var result = await db.update(repositorySortByStar, row,);
    // print('Update RepoList result: $result');
    return result;
  }

  Future minPrimaryKeySBS() async {
    Database db = await DatabaseHelper().database;
  ...
  Future deleteRepoListSortByStarTable()async{
    Database db = await DatabaseHelper().database;
    var result = await db.delete(repositorySortByStar);
