...
class RepoListTable {
  ...
  Future getRepoList() async {
    ...
    var result = await db.query(repository, orderBy: '$primeKey ASC');
    print('Get RepoList result: $result');
    return result;
  }


  Future insertRepoList({
    int? primeKeyRepo,
    int? idRepo,
    String? nodeIdRepo,
    String? nameRepo,
    ...
    var result = await db.insert(repository, row,conflictAlgorithm: ConflictAlgorithm.replace);
    print('Insert RepoList result: $result');
    return result;
  }
  getRepoByPrimaryKey(int key) async {
    Database db = await DatabaseHelper().database;
    var result = await db.rawQuery("SELECT * FROM $repository WHERE $primeKey = $key");
    // print("getRepoByPrimaryKey:- result = $result");
    return result;
  }
  getRepoByPrimaryKeys({required int fromKey, required int toKey}) async {
    Database db = await DatabaseHelper().database;
    var result = await db.rawQuery("SELECT * FROM $repository WHERE $primeKey BETWEEN $fromKey AND $toKey");
    // print("getRepoByPrimaryKeys:- fromKey = $fromKey and toKey = $toKey");
    return result;
  }
  getRepoByURL(String repoUrl) async {
    Database db = await DatabaseHelper().database;
    var result = await db.rawQuery("SELECT * FROM $repository WHERE $url = '${repoUrl.toString()}'");
    // print("getRepoByPrimaryKey:- result = $result");
    return result[0];
  }
  Future updateRepo({int? idRepo}) async {
    Database db = await DatabaseHelper().database;
    var row = {
      id: idRepo
    };
    var result = await db.update(repository, row,);
    // print('Update RepoList result: $result');
    return result;
  }

  Future minPrimaryKey() async {
    Database db = await DatabaseHelper().database;
  ...
  Future deleteRepoListTable()async{
    Database db = await DatabaseHelper().database;
    var result = await db.delete(repository);
